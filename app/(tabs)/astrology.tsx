...
/* -------------------------
 * Hemisphere normalisation
 * ------------------------- */
type HemiLong = 'Northern' | 'Southern';

function normalizeHemisphere(input: any): HemiLong {
  const s = asString(input).trim().toLowerCase();

  if (s === 'nh' || s === 'n' || s === 'north' || s === 'northern' || s === 'northern hemisphere') {
    return 'Northern';
  }
  if (s === 'sh' || s === 's' || s === 'south' || s === 'southern' || s === 'southern hemisphere') {
    return 'Southern';
  }

  // Default to Southern for your primary audience if unknown
  return 'Southern';
}

/* -------------------------
 * Cusp detection and label normalisation
 * ------------------------- */
const ZODIAC = [
  'aries',
  'taurus',
...
function isCuspLabel(labelRaw: string): boolean {
  ...
  // Common cusp formats in your DB: Aquarius_pisces, Pisces-Aries, Pisces–Aries
  const cleaned = s.replace(/[\s]/g, '');
  const hasSeparator = cleaned.includes('_') || cleaned.includes('-') || cleaned.includes('–');

  if (!hasSeparator) return false;

  // Count zodiac words present
  let hits = 0;
...
/* -------------------------
 * Build user object for daily cache fetch
 * ------------------------- */
function buildDailyUser(base: any, signLabel: string, hemi: HemiLong) {
  const cusp = isCuspLabel(signLabel);
  return {
    id: base?.id,
    email: base?.email,
    hemisphere: hemi,
    preferred_sign: signLabel,
    cuspResult: cusp ? { isOnCusp: true, cuspName: signLabel } : { isOnCusp: false, primarySign: signLabel },
  };
}

/* -------------------------
 * Cusp label normalisation for gemstone lookup
 * ------------------------- */
function normaliseCuspLabelForLookup(labelRaw: string): string {
  ...
  // Normalise dash variants to hyphen first
  s = s.replace(/[—–]/g, '-');
...
function getCuspGemstoneSafe(labelRaw: string) {
  const original = asString(labelRaw).trim();
  if (!original) return null;

  const normalised = normaliseCuspLabelForLookup(original);

  // Candidate variants that match your cuspData keys in practice
  const candidates = Array.from(
    new Set([
      original,
      normalised,
      `${normalised} Cusp`,
      // Sometimes cuspData uses hyphen, sometimes en dash
      normalised.replace(/-/g, '–'),
      `${normalised.replace(/-/g, '–')} Cusp`,
      // Extra strict collapse around separators
      normalised.replace(/\s*-\s*/g, '-'),
      `${normalised.replace(/\s*-\s*/g, '-')} Cusp`,
      normalised.replace(/\s*-\s*/g, '–'),
      `${normalised.replace(/\s*-\s*/g, '–')} Cusp`,
    ])
  );
...
export default function AstrologyScreen() {
  ...
  const [selectedHemisphere, setSelectedHemisphere] = useState<HemiLong>('Southern');
  ...
  const resolvedSign = useMemo(() => {
    ...
    if (user?.cuspResult) {
      // Be defensive: some profiles might not have isOnCusp
      const label = (user as any).cuspResult?.cuspName || (user as any).cuspResult?.primarySign;
      return asString(label) || undefined;
    }
    return undefined;
  }, [user, params.sign]);

  const resolvedHemisphere = useMemo<HemiLong>(() => {
    // Params hemisphere first, then provider, then profile
    const p = asString(params.hemisphere);
    if (p) return normalizeHemisphere(decodeURIComponent(p));

    if (contextHemisphere) return normalizeHemisphere(contextHemisphere);

    return normalizeHemisphere((user as any)?.hemisphere);
  ...
  const filteredEvents = useMemo(() => {
    ...
    if (resolvedHemisphere === 'Northern') {
      return (astronomicalEvents || []).filter(
        (e) => !southernOnly.test(`${e?.name || ''} ${e?.description || ''}`)
      );
    }
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const hemi = normalizeHemisphere(resolvedHemisphere);
        const dailyUser = buildDailyUser(user, effectiveSign, hemi);
        ...
        try {
          const consts = await getVisibleConstellationsEnhanced(hemi);
      ...
      try {
        ...
        setUser((prev) => {
          ...
          const same =
            !!prev &&
            prev.email === u?.email &&
            (prev as any).hemisphere === (u as any)?.hemisphere &&
            prevAny?.cuspResult?.cuspName === uAny?.cuspResult?.cuspName &&
            prevAny?.cuspResult?.primarySign === uAny?.cuspResult?.primarySign;
        ...
        const hemiParamRaw = asString(params.hemisphere) ? decodeURIComponent(asString(params.hemisphere)) : '';

        const signResolved =
          signParam ||
          ((u as any)?.cuspResult?.cuspName ||
            (u as any)?.cuspResult?.primarySign ||
            (u as any)?.preferred_sign ||
            '');

        const hemiResolved: HemiLong = normalizeHemisphere(
          hemiParamRaw || contextHemisphere || (u as any)?.hemisphere || 'Southern'
        );
        ...
        // These must use normalised hemisphere strings
        const lunar = getLunarNow(hemiResolved);
        const events = getHemisphereEvents(hemiResolved);

        let positions: any[] = [];
        try {
          positions = await getCurrentPlanetaryPositionsEnhanced(hemiResolved);
        } catch (e) {
          console.warn('[astrology] planetary positions error:', e);
          positions = [];
        }

        setMoonPhase(lunar || null);
        setAstronomicalEvents(Array.isArray(events) ? events : []);
  ...
  }, [serviceDateUTC, params.sign, params.hemisphere, contextHemisphere, router]);
  ...
  useEffect(() => {
    const run = async () => {
      ...
      try {
        ...
        if (daily.affirmation) translations.affirmation = await translateText(stripVersionSuffix(daily.affirmation), currentLanguage);
        if (daily.deeper) translations.deeper = await translateText(stripVersionSuffix(daily.deeper), currentLanguage);
  ...
  const getDisplayText = (original?: string) => {
    ...
    if (daily?.affirmation && base === stripVersionSuffix(daily.affirmation)) return translatedContent.affirmation ?? base;
  ...
  const hemiForPanels = normalizeHemisphere(resolvedHemisphere);
  const cusp = isCuspLabel(effectiveSign);

  return (
    <View style={styles.container}>
      <CosmicBackground />
      <MysticMish hemisphere={hemiForPanels} />
      <SafeAreaView style={styles.safeArea}>
        <ScrollView
        ...
        >
          ...
          <Text style={styles.hemisphereDisplay}>{hemiForPanels} Hemisphere</Text>
          <HemisphereToggle />
          ...
          {daily?.daily ? (
            ...
            <LinearGradient
              colors={['rgba(139, 157, 195, 0.12)', 'rgba(139, 157, 195, 0.06)']}
              style={styles.horoscopeCard}
            >
              <View style={styles.cardHeader}>
                <Star size={20} color="#8b9dc3" />
              ...
              <Text style={styles.horoscopeText}>
                No daily guidance found for your sign and hemisphere today. Pull to refresh.
              </Text>
            </LinearGradient>
          )}
          ...
          {visibleConstellations.length > 0 && (
            ...
            >
              ...
              <Text style={styles.constellationsNote}>
                Based on the current season in the {hemiForPanels} Hemisphere
              </Text>
            </LinearGradient>
          )}
