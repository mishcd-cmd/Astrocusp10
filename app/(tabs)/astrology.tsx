...
function normaliseCuspLabelForLookup(labelRaw: string): string {
  // Keep this extremely conservative. We only strip obvious suffix noise.
  // Goal: convert "Pisces–Aries Cusp" to "Pisces–Aries"
  let s = asString(labelRaw).trim();

  // Normalise dash variants to en dash first for consistency
  // Includes hyphen, en dash, em dash
  s = s.replace(/[—–-]/g, '–');

  // Remove trailing "Cusp" (case-insensitive), and any extra spaces
...
function getCuspGemstoneSafe(labelRaw: string) {
  ...
  // Try a few safe variants that commonly differ between datasets
  const candidates = Array.from(
    new Set([
      original,
      normalised,
      // Variant with hyphen instead of en dash
      normalised.replace(/[—–]/g, '-'),
      // Variant with en dash (already) but ensure spaces around dash are consistent
      normalised.replace(/\s*–\s*/g, '–'),
      // Variant with spaces around hyphen
      normalised.replace(/\s*–\s*/g, ' - ').replace(/\s+/g, ' ').trim(),
      // Sometimes stored with no spaces and simple hyphen
      normalised.replace(/\s*–\s*/g, '-'),
    ])
  );
        ...
        >
          ...
          {hasAccess && isCusp && (
            ...
            >
              ...
              {(() => {
                // IMPORTANT: do not change sign selection logic, only normalise for lookup
                ...
                // Helpful debug so you can see the label mismatch in logs
