...
// Normalize and lock your site URL to a single safe origin
function normalizeSiteUrl(raw?: string): string {
  // default to your real domain
...
// Only allow return URLs that match the same origin as site
  ...
  try {
    ...
    if (existingCustomer?.customer_id) {
      ...
      try {
        ...
        return new Response(JSON.stringify({ error: `Failed to create customer: ${stripeError.message}` }), {
          status: 500,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        })
      }
    ...
    // Build safe base and defaults
    const SITE = normalizeSiteUrl(Deno.env.get('SITE_URL'))
    const DEFAULT_SUCCESS = `${SITE}/settings?status=success`
    const DEFAULT_CANCEL = `${SITE}/settings?status=cancel`

    // If caller sent URLs, accept them only if same origin as SITE
